...
class atmsCLIPVisionTransformer(CLIPVisionTransformer):
    def __init__(
        self,
        atms_config=None,
        **kwargs,
    ...
    def forward(self, x: torch.Tensor):
        ...
        for i, blk in enumerate(self.transformer.resblocks):
            x = blk(x)
            x = self.atms.forward(x, i, batch_first=False, has_cls_token=True)
    ...
    def train(self, mode: bool = True):
        ...
        set_requires_grad(self, ["fpn", "atms"])
        set_train(self, ["fpn", "atms"])

    def state_dict(self, destination, prefix, keep_vars):
        state = super().state_dict(destination, prefix, keep_vars)
        keys = [k for k in state.keys() if ("atm" not in k) and ('fpn' not in k)]
        for key in keys:
            state.pop(key)
